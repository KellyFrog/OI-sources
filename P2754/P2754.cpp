...
int main() {
	...
	rep(i, 1, m) {
		int x; cin >> c[i] >> x;
		v[i].resize(x);
		for(int& a : v[i]) cin >> a, a += 2, merge(a, v[i][0]);
	}
	...
	int tt = 1;
	int sum = 0;
	rep(i, 1, n+2) id2[i] = ++iid;
	addedge(s, id2[2], inf);
	addedge(id2[2], s, 0);
	for(; (sum += dinic()) < k; ++tt) {
		rep(i, 1, n+2) id1[i] = id2[i];
		rep(i, 1, n+2) {
			id2[i] = ++iid;
			addedge(id1[i], id2[i], inf);
			addedge(id2[i], id1[i], 0);
		}
		rep(i, 1, m) {
			int p = (tt-1)%v[i].size();
			int to = tt%v[i].size();
			addedge(id1[v[i][p]], id2[v[i][to]], c[i]);
			addedge(id2[v[i][to]], id1[v[i][p]], 0);
		}
		addedge(id2[1], t, inf);
		addedge(t, id2[1], 0);
	}
