// Problem: CF1136D Nastya Is Buying Lunch
// Contest: Luogu
// URL: https://www.luogu.com.cn/problem/CF1136D
// Memory Limit: 250 MB
// Time Limit: 2000 ms
...
int main() {
	...
	sort(npos.begin(), npos.end(), [&](const int& u, const int& v) { return u > v; });
	
	int ans = 0;
	
	for(int p : npos) {
		int cnt = 0;
		for(int v : adj[p]) if(!vis[v] && v > p) cnt++;
		if(cnt + query(p, n) == n - p) {
			ans++;
			add(p, 1);
			vis[p] = 1;
		}
