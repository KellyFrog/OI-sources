// Problem: U176532 frog
// Contest: Luogu
// URL: https://www.luogu.com.cn/problem/U176532
// Memory Limit: 128 MB
// Time Limit: 1000 ms
...
bool check() {
	int lpos = -1, rpos = -1;
	rep(i, 1, n) if(a[i] == 1) { lpos = i; break; }
	per(i, 1, n) if(a[i] == 1) { rpos = i; break; }
	if(lpos == -1) return 1;
	rep(i, lpos, rpos) if(a[i] == 0) return 0;
	return 1;
}

void dfs(int step, int pos) {
	if(step >= ans) return;
	if(check()) return chkmin(ans, step), void();
	if(pos > 1){
		swap(a[pos], a[pos - 1]);
		dfs(step + 1, pos - 1);
		swap(a[pos], a[pos - 1]);
	...
	if(pos > 2){
		a[pos - 2] ^= 1, swap(a[pos], a[pos - 2]);
		dfs(step + 1, pos - 2);
		swap(a[pos], a[pos - 2]), a[pos - 2] ^= 1;
	}
	if(pos + 2 <= n){
		a[pos + 2] ^= 1, swap(a[pos], a[pos + 2]);
		dfs(step + 1, pos + 2);
