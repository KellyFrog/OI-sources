// Problem: U176042 Factory2
// Contest: Luogu
// URL: https://www.luogu.com.cn/problem/U176042
// Memory Limit: 128 MB
// Time Limit: 1000 ms
...
int LCA(int x, int y) {
	...
	per(i, 0, 17) if(dep[anc[x][i]] >= dep[y]) x = anc[x][i];
	if(x == y) return x;
	per(i, 0, 17) if(anc[x][i] != anc[y][i]) x = anc[x][i], y = anc[y][i];
	return anc[x][0];
}
...
int main() {
	...
	while(Q--) {
		...
		if(op == 'R') {
			...
			dep[n] = dep[x] + 1;
			anc[n][0] = x;
			rep(i, 1, 17) anc[n][i] = anc[anc[n][i-1]][i-1];
		} else if(op == 'C') {
			...
			if(a[x] == 0) a[x] = b[x] = y;
				...
				ll dis1 = c[x];
				ll dis2 = getdis(a[x], y);
				ll dis3 = getdis(b[x], y);
				ll mxdis = max(max(dis1, dis2), dis3);
				if(dis2 == mxdis) {
					c[x] = mxdis;
					b[x] = y;
				} else if(dis3 == mxdis) {
			...
			int x, y;
			cin >> x >> y;
			cout << max(max(getdis(a[x], a[y]), getdis(a[x], b[y])), max(getdis(b[x], a[y]), getdis(b[x], b[y]))) << "\n";
		}
	}
