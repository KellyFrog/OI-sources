...
int x[N], y[N], z[N];
int n, h, r;
int que[N], phead, ptail;
bool vis[N];

bool check(int i, int j) {
	ll dist = 1ll * (x[i] - x[j]) * (x[i] - x[j]) + 1ll * (y[i] - y[j]) * (y[i] - y[j]) + 1ll * (z[i] - z[j]) * (z[i] - z[j]);
	return dist <= 4ll * r * r;  
}
...
void solve() {
	...
	rep(i, 1, n) if(z[i] <= r) vis[i] = 1, que[++ptail] = i;
	while(phead <= ptail) {
		int u = que[phead++];
		rep(v, 1, n) {
			if(!vis[v] && check(u, v)) {
	...
	bool ok = 0;
	rep(i, 1, n) if(z[i] >= h - r && vis[i]) ok = 1;
	cout << (ok ? "Yes" : "No") << "\n";
}
