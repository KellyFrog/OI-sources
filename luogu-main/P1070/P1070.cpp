// Problem: P1070 [NOIP2009 普及组] 道路游戏
// Contest: Luogu
// URL: https://www.luogu.com.cn/problem/P1070
// Memory Limit: 125 MB
// Time Limit: 1000 ms
...
int main() {
	...
	cin >> n >> m >> p;
	rep(i, 1, n) rep(j, 1, m) cin >> a[i%n][j];
	rep(i, 0, n-1) cin >> b[i];
	rep(i, 1, m) {
		rep(j, 0, n-1) pre[i][j] = pre[i-1][(j-1+n)%n] + a[j][i];
	}
	...
	rep(i, 1, m) {
		rep(j, 0, n-1) {
			...
			rep(k, 1, t) {
				...
				pp %= n;
				if(pp < 0) pp += n;
				f[i] = max(f[i], f[i-k] + pre[i][j] - pre[i-k][pp] - b[pp]);
			}
		}
	}
	cout << f[m] << "\n";
