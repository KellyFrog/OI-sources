...
struct BIT {
	...
	inline void modify(int x, int val) { for(int x0 = x; x0 <= (int)1e5; x0 += lowbit(x0)) chkmax(t[x0], val); }
	inline int query(int x) { int ret = 0; for(int x0 = x; x0; x0 -= lowbit(x0)) chkmax(ret, t[x0]); return ret; }
};
...
int main() {
	...
	rep(i, 1, n) {
		f[i] = t1.query(a[i] - 1) + 1; t1.modify(a[i], f[i]);
		g[i] = t2.query((1e5 - a[i] + 1) ) + 1; t2.modify(1e5 - a[i] + 1, g[i]);
		cerr << a[i] << " " << f[i] << "\n";
		chkmax(ans1, f[i]);
		chkmax(ans2, g[i]);
	}
