...
int main() {
	scanf("%s", s+1);
	n = strlen(s + 1);
	rep(i, 1, n) if((s[i-dp[i-1]-1] == '(' && s[i] == ')') || (s[i-dp[i-1]-1] == '[' && s[i] == ']')) dp[i] = dp[i-1] + 2 + dp[i-dp[i-1]-2];
	int ans = 0, pos = 0; rep(i, 1, n) if(dp[i] > ans) ans = dp[i], pos = i;
	rep(i, pos-dp[pos]+1, pos) printf("%c", s[i]);
	return puts(""), 0;
}
