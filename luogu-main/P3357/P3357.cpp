...
int main() {
	...
	rep(i, 1, n) {
		int x, y;
		cin >> l[i] >> x >> r[i] >> y;
		c[i] = sqrt(1ll*(r[i]-l[i])*(r[i]-l[i])+1ll*(y-x)*(y-x));
		assert(c[i] >= 0);
	...
	sort(a+1, a+pa+1);
	int m = pa = unique(a+1, a+pa+1) - a-1;
	rep(i, 1, n) {
		l[i] = lower_bound(a+1, a+pa+1, l[i]) - a;
		r[i] = lower_bound(a+1, a+pa+1, r[i]) - a;
		assert(l[i] < r[i]);
