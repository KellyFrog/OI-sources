...
// Memory Limit: 512 MB
// Time Limit: 1000 ms
// Create Time: 2022-04-12 22:28:40
...
int main() {
	...
	f[0] = 1;
	rep(i, 1, m) f[i] = 1ll * f[i-1] * (n-i+1) % mod;
	int ans = 0;
	rep(i, 0, m) rep(j, 0, i) {
		ans = (ans + 1ll * a[i] * s[i][j] % mod * qpow(1 + x, n - j) % mod * f[j] % mod * qpow(x, j)) % mod;
	}
	cout << ans << "\n";
